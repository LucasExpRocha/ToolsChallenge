...
class PaymentDbIntegrationTest {
  ...
  void persisteCamposComSeguranca() {
    ...
    Payment p = repository.findByExternalId("100023568900020").orElseThrow();
    assertEquals("4444123412341234", p.getCartao());
    assertEquals("AVISTA", p.getTipo());
    assertEquals(Integer.valueOf(1), p.getParcelas());
    assertEquals("PetShop Mundo cão", p.getEstabelecimento());
    assertNotNull(p.getNsu());
    assertNotNull(p.getCodigoAutorizacao());
  }
}